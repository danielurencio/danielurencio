...
function graph(error, D09) {
  
  if (error) throw error;

	var country = [];
	D09.forEach(function(d) {   // Filtro de países
		if (d.country == "China") {
		    country.push(d);
		}
...
//	    "height": height,
	...
	.text("Usuarios de internet p/c 100 personas")
	.style(title);

        ts(D09, "body", config);
    enlist( feats(D09),"body");
