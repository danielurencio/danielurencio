...
function graph(error, D12) {
  
  if (error) throw error;

	var country = [];
	D12.forEach(function(d) {   // Filtro de países
		if (d.country == "China") {
		    country.push(d);
		}
...
//	    "height": height,
	...
	.text("Consumo de energía de combustibles fósiles (%)")
	.style(title);

        ts(D12, "body", config);
    enlist( feats(D12),"body");
